...
MqttClient.prototype.publish = function (topic, message, opts, callback) {
  ...
  var publishProc = function () {
    ...
    if (opts.qos === 1 || opts.qos === 2) {
      ...
      if (messageId === null) {
        ...
        that.emit('error', new Error('No messageId left'))
        return that
      }
    }
    packet = {
  ...
  if (this._storeProcessing) {
    this._storeProcessingQueue.push(publishProc)
  } else {
    publishProc()
  }
  return this
...
MqttClient.prototype.subscribe = function () {
  ...
  if (messageId === null) {
    debug('No messageId left')
    this.emit('error', new Error('No messageId left'))
    return this
  }
...
MqttClient.prototype.getLastMessageId = function () {
  return this.messageIdProvider.getLastMessageId()
}
...
MqttClient.prototype._onConnect = function (packet) {
  ...
  function startStreamProcess () {
    ...
    outStore.on('end', function () {
      ...
      if (allProcessed) {
        clearStoreProcessing()
        that.removeListener('close', remove)
        for (var f of that._storeProcessingQueue) {
          f()
        }
        that._storeProcessingQueue.splice(0)
  ...
  if (!packet.sessionPresent) {
    that.outgoingStore.clear(function () {
      startStreamProcess()
    })
  } else {
    startStreamProcess()
